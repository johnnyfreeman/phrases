...
canPhrase = function(user, returnError){
  var user=(typeof user === 'undefined') ? Meteor.user() : user;

  // console.log('canPhrase', user, action, getSetting('requirePhraseInvite'));
  if(Meteor.isClient && !Session.get('settingsLoaded'))
    return false;
  ...
  if(!user){
    ...
    return true;
  } else if (getSetting('requirePhraseInvite')) {
    if (user.isInvited) {
      return true;
    } else {
      return returnError ? "no_invite" : false;
...
canPhraseById = function(userId, returnError){
  var user = Meteor.users.findOne(userId);
  return canPhrase(user, returnError);
}
